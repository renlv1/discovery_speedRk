  ...
  <transition name="safeDialog">
    <div class="dialog-w dialog-safe-w">
      <div class="dialog-safe" >
        ...
        <div class="dialog-title">{{$t('safe.text50')}}</div>
        <div class="form-item">
          <input type="text" :placeholder="$t('safe.text11')" class="input" v-model.trim="payCode">
        </div>
  ...
  export default {
    ...
    methods: {
      ...
      okCallback() {
        this.errInputMsg = ''
        if (this.payCode === '') {
          this.errInputMsg = this.$t('safe.text14')
          return
        }
        if (this.payCode !== '') {
          this.$fetch.post('/tx/safetyvalidate', {
            mobileCode: this.payCode,
            codeType: 1
          }).then(res => {
            if (res.success) {
              this.$emit('succeed', this.payCode)
            } else {
