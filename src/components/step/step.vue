  ...
  <div class="steps" :data="stepData">
    ...
    <ul>
      <li class="item" v-for="item in stepData" :key="item.index"
        ...
        <div v-clipboard:copy=stepData[3].colorMsg v-clipboard:success="clipboardSuccess" class="btn"
             v-if="item.index === 4 && (item.status === 1 || item.status === 2)" v-waves>{{$t('complaint.complaintContent.text8')}}
        </div>
      </li>
    </ul>
  </div>
