  ...
  <transition name="safeDialog">
    <div class="dialog-w dialog-safe-w">
      <div class="dialog-safe" >
        ...
        <div class="form-item">
          <div class="inupt-box">
            <input @input="inputFn" maxlength="18" :type="inputType" :placeholder="$t('market.text29') + ' ' + rateTitle + ' ' + $t('market.text30')" class="input" v-model.trim="payCode">
          </div>
        </div>
  ...
  export default {
    ...
    methods: {
      ...
      okCallback() {
        this.errInputMsg = ''
        if (this.payCode === '') {
          this.errInputMsg = this.$t('sharer.contactWay.text7')
          return
        }
        if (this.payCode !== '') {
          this.$emit('succeedContact', this.payCode,this.errInputMsg)
        }
      },
      inputFn () {
        if (this.rateTitle === 'qq') {
