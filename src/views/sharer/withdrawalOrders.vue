  ...
  <div class="withdrawalOrders-wrap">
    ...
    <div class="top-box">
      ...
      <div class="search-box">
        ...
        <input class="input" type="number" maxlength="18" v-model.trim="search" @input="searchInputFn()"  :placeholder="this.$t('withdrawalOrder.text4')" autocomplete="off"/>
        <div class="close" :class="{'active': closeShow}" @click="closeFn()"><img src="../../assets/img/order/close.svg" alt=""></div>
      </div>
    </div>
    <div class="main-c" :class="{'active': nullDataShow}">
      <!--提现订单数据列表-->
      <div class="list-box" :class="{'active': activeShow}" v-if="boxShow">
        <cube-scroll
          ref="cubeListSharer"
          :options="options"
          ...
          <div class="list-item" @click="itemFn(item)" v-for="(item,index) in withdrawalOrderList" :key="index">
            <div class="info-box">
              <div class="lrs">
                ...
                <div class="account-name"><span>{{item.userNick}}</span><span class="audit-time">{{$changeDate(item.createAt, '.')}}</span></div>
              </div>
            </div>
            <div class="hr"></div>
            <div class="receivable-box">
              ...
              <div class="receivable">{{$t('topUp.topUpList.item.text3')}}</div>
      ...
      <!--搜索订单列表-->
      <div class="list-box" :class="{'active': activeShow}" v-else-if="!boxShow">
  ...
  export default {
    ...
    methods: {
      ...
      inputValueFn () {
        ...
        if (search) {
          this.$fetch.post(`${this.$api}draw/queryMyOrderList`,{
            id: search,
      ...
      closeFn () {
        ...
        this.pageIndex = 1
        this.emptyShow = false
        this.emptyText = this.$t('withdrawalOrder.text7')
        clearTimeout(this.timer)
        this.timer = setTimeout(() => {
          this.theOrderListFn()
        }, 200)
      ...
      // 查询提现订单列表数据
      /*1.82 查询提现记录
      接口URL: http://boss.pigamegroup.com/draw/queryMyOrderList*/
      theOrderListFn () {
        this.$fetch.post(`${this.$api}draw/queryMyOrderList`,{
          ...
          if (res.status === "success") {
            ...
            this.$refs.cubeListSharer.forceUpdate()
          }
        }).catch((err) => {
          console.log(err)
        })
      ...
      // 获得列表详情
      itemFn (item) {
        this.$router.push({path: '/theOrderDetails', query:{detailsdId: item.id, isChongzhi: 1}})
      }
    }
  }
</script>
